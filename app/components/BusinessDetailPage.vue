...
import { citySegmentFor, businessUrl } from '~/utils/urls'
...
const citySegment = computed(() => String(route.params.city ?? '').toLowerCase())
const { negocio, pending, error, refresh: refreshNegocio } = useNegocio(slug)

// Si la ciudad de la URL no coincide con la del negocio, hacemos silent
// redirect al slug correcto (solo cliente). El canonical siempre apunta a
// la URL correcta, así que Google no indexa duplicados aunque llegue tráfico
// a la variante equivocada.
watchEffect(() => {
  if (!import.meta.client || !negocio.value) return
  const expected = citySegmentFor(negocio.value)
  if (citySegment.value !== expected) {
...
// Canonical siempre apunta a la URL correcta del negocio (usando el helper),
// independientemente del segmento de ciudad que aparezca en route.path. Esto
// previene contenido duplicado si alguien accede con una ciudad equivocada.
const seoUrl = computed(() => negocio.value
  ? `${siteUrl}${businessUrl(negocio.value)}`
  : `${siteUrl}${route.path}`,
)
...
const jsonLdScripts = computed(() => {
  ...
  const seg = citySegmentFor(negocio.value)
  const crumbs = [
    { name: 'Inicio', url: `${siteUrl}/` },
    { name: negocio.value.city?.name ?? 'Jalisco', url: `${siteUrl}/${seg}` },
    ...(negocio.value.category
      ? [{ name: negocio.value.category.name, url: `${siteUrl}/${seg}/${negocio.value.category.slug}` }]
      : []),
      ...
      <div class="bg-white border-b border-gray-100 px-6 md:px-12 py-3">
        <div class="max-w-6xl mx-auto">
          <nav class="flex items-center gap-2 text-sm">
            ...
            <a :href="`/${citySegmentFor(negocio)}`" class="text-brand-primary hover:underline">{{ negocio.city?.name ?? 'Jalisco' }}</a>
            <span class="text-gray-400">›</span>
            <a
              v-if="negocio.category"
              :href="`/${citySegmentFor(negocio)}/${negocio.category.slug}`"
              class="text-brand-primary hover:underline"
            >{{ negocio.category.name }}</a>
            <span v-else class="text-brand-azulgris">Directorio</span>
            <span class="text-gray-400">›</span>
